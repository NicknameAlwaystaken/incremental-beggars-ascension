...
class GamesDropdownView(DropdownBaseView):
    def __init__(self, cog, user_id, player, trigger_function):
        super().__init__(cog, user_id)

        self.add_item(GamesDropdown(trigger_function))
        ...
        if player_coins:
            for chip_amount in chips_list:
                ...
                add_chips_button.callback = partial(self.cog.buy_chips_callback, amount=chip_amount)
...
class RPSView(GameBaseView):
    def __init__(self, cog, user_ids):
        super().__init__(cog, user_ids)

        self.create_rps_view()

    def create_rps_view(self):
        self.add_accept_button(self.cog.accept_challenge_callback)
        self.add_decline_button(self.cog.decline_challenge_callback)
...
class StartChallengeView(GameBaseView):
    def __init__(self, cog, user_ids):
        ...
        self.create_challenge_view()

    def create_challenge_view(self):
        make_choice_button = discord.ui.Button(label='Make choice', style=discord.ButtonStyle.success, row=1)
        make_choice_button.callback = self.cog.rps_choice_callback
...
class RematchChallengeView(GameBaseView):
    def __init__(self, cog, user_ids):
        ...
        self.create_rematch_challenge_view()

    def create_rematch_challenge_view(self):
        rematch_button = discord.ui.Button(label='Rematch?', style=discord.ButtonStyle.success, row=1)
        rematch_button.callback = self.cog.game_rematch_callback
